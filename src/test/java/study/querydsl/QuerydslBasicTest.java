...
public class QuerydslBasicTest {
    ...
    //세타조인 : 연관관계가 없는 상황에서 막조인(풀조인) 해놓고, where절에서 제한
    @Test
    ...
    public void fetchJoinNo() throws Exception {
        ...
        // team이 영속성 컨텍스트에 로딩된 엔티티인지 아닌지 확인하는 기능
    ...
    public void concat() throws Exception {
        // given
        List<String> fetchs = queryFactory.select(member.username.concat("_").concat(member.age.stringValue()))
                .from(member)
                .where(member.username.eq("member1"))
                .fetch();
    ...
    // select절에 나열하는 것을 Projection이라 한다.
    @Test
    ...
    public void bulkUpdate() throws Exception {
        ...
        for (Member memberDto : resultList) {
            System.out.println("memberDto = " + memberDto);
        }
    }
